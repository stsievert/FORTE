...
import FORTE.objectives.L1Loss as L1
cimport FORTE.objectives.L1Loss as L1
...
def computeEmbedding(int n, int d,S,num_random_restarts=0,
    ...
    for restart in range(num_random_restarts + 1):
        ...
        te_sgd = time()-ts
        ts = time()
        X_new,emp_loss_new,l1_loss_new, _ = computeEmbeddingWithFG(X,S,
        ...
        if verbose:
            print ("restart %d:   emp_loss = %f,   "
                   "l1_loss = %f,   duration=%f+%f") %(restart,
                                                          emp_loss_new,l1_loss_new,
                                                          te_sgd,te_gd)
    return X_old
...
def computeEmbeddingWithEpochSGD(int n,int d,S,max_num_passes=0,max_norm=0,epsilon=0.001,a=1,verbose=False):
    ...
    cdef double score, outer_loss, emp_loss, l1_loss, norm_grad_sq_0
    ...
    while t < max_iters:
        ...
        X[q,:] = X[q,:] - a*L1.partialGradient(X,q)[q,:]		# take SGD step
...
def computeEmbeddingWithFG(np.ndarray[DTYPE_t, ndim=2] X,S, max_iters=50,max_norm=1.,epsilon=0.01,c1=.00001,rho=0.5,verbose=False):
    ...
    Usage:
        X,emp_loss,l1_loss,gamma = computeEmbeddingWithFG(X,S)
    """
    cdef int n = X.shape[0]
    cdef int d = X.shape[1]
    cdef double alpha = 100.
    cdef double emp_loss, l1_loss
    emp_loss = utils.empirical_lossX(X,S)
    l1_loss = L1._l1Loss(X,S)
    cdef int t = 0
    cdef int inner_t = 0
    cdef double l1_loss_0 = L1._l1Loss(X,S)
    cdef double rel_max_grad = float('inf')

    while t < max_iters:
        t = t + 1
        # get gradient and stopping-time statistics
        G = L1.fullGradient(X,S)
        rel_max_grad, norm_grad_sq_0 = relative_grad(G,X)
        if rel_max_grad < epsilon:
            break
        # perform backtracking line search
        l1_loss_k = L1._l1Loss(X-alpha*G,S)
        alpha = 1.1*alpha
        inner_t = 0
        while l1_loss_k > l1_loss_0 - c1*alpha*norm_grad_sq_0:
        # while l1_loss_k > l1_loss_0:    
            alpha = alpha*rho
            log_loss_k = L1._l1Loss(X-alpha*G,S)
            inner_t += 1
            if inner_t > 4:
                break
        X = X-alpha*G 
        l1_loss_k = L1._l1Loss(X,S)
        if inner_t == 0:
            alpha = 2*alpha
        l1_loss_0 = l1_loss_k     # save previous log_loss for next line search
        ...
        blackbox.logdict({'iter':t,
                  ...
                  'inner_t':inner_t})
        blackbox.save(verbose=verbose)                    
    return X,emp_loss,l1_loss_0,rel_max_grad
